...
const OrderSummary = ({ totalPrice, items }) => {
  ...
  useEffect(() => {
    if (getToken) {
      dispatch(fetchAddress({ getToken })); // Fetches user's saved addresses from database ...Stores them in Redux state
    }
  }, [dispatch, getToken]);
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      ...
      if (paymentMethod === "STRIPE") {
        ...
        router.push("/orders");
        dispatch(fetchCart({ getToken })); //it will fetch cart data again from database & the cart data will be empty
      }
    } catch (error) {
          ...
          <div className="flex flex-col gap-1 font-medium text-right">
            ...
            <p>
              <Protect plan={"plus"} fallback={`${currency}5`}>
                ...
                {/*fallback={${currency}5} inside <Protect /> means: If the user does NOT have the "plus" plan, it will show $5 (or your currency + 5) instead of "Free" */}
                Free
              </Protect>
            </p>
            {coupon && (
