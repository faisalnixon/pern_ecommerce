...
export async function POST(request: NextRequest) {
  try {
    const { has, userId } = getAuth(request);
    if (!userId) {
    ...
    const ordersByStore = new Map<string, OrderItemType[]>();
    ...
    console.error(error);
    return NextResponse.json(
      { errror: error.code || error.message },
      { status: 400 },
    );
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error(error);
    return NextResponse.json({ errror: error.message }, { status: 400 });
  }
}
